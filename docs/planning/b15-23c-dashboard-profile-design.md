...
Die spätere E5-Live-Regression zeigte bei Passwort-Recovery eine veraltete Basis-URL: Die zentrale Redirect-Hilfe bevorzugte konfigurierte Werte vor der tatsächlich im Browser aufgerufenen Origin. Dadurch konnte trotz Tunnelaufruf ein `https://localhost:3000`-Callback entstehen, während der lokale Dev-Server nur HTTP anbietet. Browser-Recovery bevorzugt nun die syntaktisch validierte aktuelle Origin; ausschließlich HTTPS beziehungsweise HTTP für Loopback-Hosts ist zulässig. Server-only Invite und E3-Bestätigung bleiben konfigurationsbasiert. Zusätzlich muss die jeweilige Callback-Origin unter **Supabase Authentication → URL Configuration → Redirect URLs** zugelassen sein; `Site URL` ist kontrolliert auf die aktuelle Deployment-/Produktivdomain zu setzen. Diese Dashboardänderung wurde nicht durch Codex vorgenommen.

Ein anschließender Live-Nachweis bestätigte, dass Supabase bereits korrekt auf den HTTPS-Tunnel-Callback leitete. Der verbleibende localhost-Sprung entstand danach in der eigenen Route: `new URL(request.url)` enthielt hinter dem lokalen Cloudflare-Upstream die interne Origin `http(s)://localhost:3000`, die anschließend für `/admin/set-password` wiederverwendet wurde. Die Callbackroute löst die öffentliche Request-Origin nun separat auf. Eine bereits öffentliche HTTPS-Origin bleibt unverändert; nur bei einer internen Loopback-Origin darf ein einzelner, syntaktisch validierter HTTPS-Forwarded-Host übernommen werden. Widersprüchliche, mehrteilige oder missgebildete Forwarded-Header werden verworfen. `next` bleibt exakt auf `/admin/set-password` begrenzt, und Environment-Werte beeinflussen den Callback-Redirect nicht.
