...
7. **B15.24G – COMPLETE – CURRENT FOOTBALL AREA ACCEPTED AS DELIVERED:** Bestehende Mannschaftsseiten mit integriertem Kader, Trainer/Ansprechpartner und Trainingszeiten bleiben unverändert. Eigenständige Spielerprofile sind nicht vorgesehen; eine separate Fußballchronik ist nicht erforderlich. FUSSBALL.DE-Widgets sind ein Domain-abhängiges Go-live-To-do, der Turniere-&-Events-Vollausbau ist Future/Post-Go-live.
...
- Browser-Nacharbeit 11 nutzt den freien Desktop-Headerbereich als kleine Serviceleiste. Instagram erscheint nur mit einer gültigen HTTP(S)-URL aus `club_settings.social_links.instagram`; Sportanlage/Anfahrt und Termine verwenden die bestehenden internen Ziele `/kontakt` und `/termine`. Mobile bleibt unverändert, weil diese Ziele über Navigation beziehungsweise Footer erreichbar sind. Der Footer besitzt nun fünf responsive Bereiche für Vereinspräsentation, Verein, Sportarten, weitere interne Links und Kontakt. Vereinsname, Adresse, Telefon, E-Mail und Social Links stammen weiterhin ausschließlich aus `club_settings`; Rechtlinks werden nur für tatsächlich publizierte `impressum`-/`datenschutz`-Seiten ausgegeben. Im Repository waren keine verifizierten URLs für DJK-Sportverband, Stadtsportbund, Heimatverein oder Gewerbekreis vorhanden, daher wurden keine externen Links ergänzt. `/agb` und eine reale Consent-Verwaltung fehlen weiterhin; weder Fake-Links noch funktionslose Controls wurden angelegt. Cookie-Einstellungen werden eingeblendet, sobald die reale Consent-Verwaltung implementiert ist. Die bestehenden NewsCards wurden bewusst nicht verändert.
- Browser-Nacharbeit 12 ersetzt die einzelne Instagram-Sonderlogik durch einen gemeinsamen URL-validierenden Social-Link-Resolver für die tatsächlich gespeicherten JSON-Keys `facebook`, `instagram`, `youtube`, `tiktok`, `linkedin` und `x`. Leere, ungültige oder mit Credentials versehene URLs werden weder im Header noch im Footer ausgegeben. „Sportanlage / Anfahrt“ führt nun auf `/anfahrt`; dort stammen Vereinsname, Adresse und `google_maps_url` aus `club_settings`. Das bestehende Maps-Feld ist freier Text ohne Embed-spezifische Validierung; der aktuelle Wert wurde read-only als normaler gültiger HTTPS-Google-Maps-Link und nicht als Embed-URL klassifiziert. Die Website akzeptiert daher nur HTTPS-Google-Maps-Ziele, unterscheidet normale Links von `/maps/embed` und lädt selbst bei einer Embed-URL erst nach einem bewussten Klick einen Drittanbieter-Iframe. Ohne Embed-URL wird ausschließlich „In Google Maps öffnen“ angeboten. `/cookie-einstellungen` erklärt den Aufbauzustand ohne Fake-Toggles oder vorgetäuschte Speicherung und verweist auf Datenschutz. Powered-by ist entfernt. Eine persistente Consent-Steuerung, AGB, Vereinsregisterdaten und verifizierte externe Verbands-/Ortslinks bleiben offen; NewsCards sind unverändert.
- Browser-Nacharbeit 13 ändert ausschließlich die Darstellung der gemeinsamen SocialLinks-Komponente auf wiedererkennbare Brandfarben: Instagram-Verlauf, Facebook- und LinkedIn-Blau, YouTube-Rot, TikTok-Cyan/Rot auf Schwarz sowie X Schwarz/Weiß. Resolver, Keys und bedingte Ausgabe bleiben unverändert. Für die offizielle Google Maps Embed API existiert nun ein zentraler fail-closed Builder. Er akzeptiert nur einen plausiblen `GOOGLE_MAPS_EMBED_API_KEY` und eine begrenzte Adressanfrage aus `club_settings`; der Konfigurationswert ist aktuell nicht vorhanden und wird weder hardcodiert noch committed. Der normale `google_maps_url` bleibt externer Fallback. Ein konfiguriertes iframe würde erst nach bewusstem Klick mit `loading="lazy"` und restriktiver Referrer-Policy geladen. Google-Cloud-Projekt, aktivierte Maps Embed API, domainbeschränkter API-Key und die spätere Consent-Anbindung bleiben offen.
- Browser-Nacharbeit 14 verwirft die originalfarbigen Brandflächen als zu dominant und vereinheitlicht Facebook, Instagram, YouTube, TikTok, LinkedIn und X auf transparente rote Glyphs mit exakt denselben `text-red-500`-/`hover:text-red-400`-Tokens wie Standort und Kalender. Die dynamische Datenlogik bleibt unverändert. Roadmap und Projektstatus führen Google Maps Inline/Embed und die echte Cookie-/Consent-Verwaltung nun als konkrete getrennte offene Folgeblöcke. Die technische Maps-Vorbereitung bleibt unangetastet; Google Cloud, Key, Inline-Aktivierung, persistente Consent-Entscheidung und Go-live-Prüfung erfolgen später gemeinsam.
- Browser-Nacharbeit 15 behebt die im Browser weiterhin weiße Darstellung: Die `react-icons`-SVGs verwenden `currentColor`, während die globale ungeschichtete Linkregel `a { color: inherit; }` den nur am Link gesetzten Tailwind-Farbwert überstimmte. `text-red-500` und `group-hover:text-red-400` liegen deshalb nun direkt am SVG. Social-Resolver, Header-/Footer-Geometrie sowie Maps-/Consent-Stand bleiben unverändert.
- Browser-Nacharbeit 16 etabliert eine gemeinsame visuelle Basis für alle öffentlichen Routen: zentraler dunkler Rot-/Anthrazit-Verlauf im Website-Root, einheitlicher Header-/Floating-Nav-Offset sowie die Primitives `PublicPageShell`, `PublicPageHero` und `PublicCard`. Normale Übersichten besitzen keinen großen äußeren Seitenkartenrahmen mehr. Vereinsbereiche verwenden dunkle gleichwertige Karten; Gesamtvorstand, Gesamtvereinsgeschichte, Behindertensport und Gymnastikdamen teilen einen strukturierten Aufbauzustand. Tischtennis, Termine, Training, Downloads, Fußball- und Mannschaftsübersichten sowie Anfahrt und Rechtseiten wurden in dieselbe Familie überführt. Alle übrigen inventarisierten öffentlichen Routen erhalten Background und Offset zentral, während Newsdetails, Formulare, Personen- und Mannschaftsdetails ihre fachlich begründeten Sonderlayouts behalten.
- Browser-Nacharbeit 17 korrigiert Navigation und fachliche History-Zuordnung. Gesamtvereins- und Header-Termine verwenden `/termine/allgemein`; die kompatible `/termine`-Auswahlroute bleibt erreichbar. Fußball entfernt den allgemeinen Termine-Punkt. Tischtennis benennt den bisherigen Termine-Einstieg in Trainingszeiten um und verwendet `/tischtennis/trainingszeiten`; `/tischtennis/termine` leitet kompatibel weiter. News verwendet in Navigation und Footer ausschließlich die vorhandene Alle-Meldungen-Seite `/news/uebersicht`. Der seit 2026 bestehende Singleton-Key `fussball-vereinsgeschichte` ist ein technischer Legacy-Key: Adminroute, Adminnavigation, Editorformulierungen und Permission-Einordnung verwalten die einzige Quelle als Gesamtvereinsgeschichte. Die veröffentlichten Titel-, Teaser-, Haupttext-, Bild- und Meilensteindaten erscheinen nun unter `/verein/vereinsgeschichte`; die alte Fußballroute leitet dorthin. Eine optionale semantische Key-Migration bleibt einem separaten DB-Block vorbehalten.
- Browser-Nacharbeit 18 optimiert die Featured-NewsCard für 360–430 px, ohne ihre bisherigen Desktopwerte zu verkleinern. Eine lokale History-Entity-Normalisierung dekodiert die bestätigten Legacy-Umlaute und `&amp;` genau einmal; alle Inhalte durchlaufen weiterhin React-Textescaping beziehungsweise den vorhandenen Richtext-Sanitizer. Der browsernative Date-Indikator des Mitgliedsantrags wird ausschließlich mobil um `0.375rem` vom rechten Rand eingerückt. Diese Punkte sind im Abschlussreview bestätigt.
- Browser-Nacharbeit 19 korrigiert den Parent-Layoutvertrag der Startseite. Die Termin-Sidebar hatte mit zwei nicht umbrechenden langen Kopftexten die Min-Content-Breite des gemeinsamen mobilen Grid-Tracks vergrößert; deshalb blieb auch eine kleiner gestaltete NewsCard breiter als der Viewport. Mobile verwendet nun einen explizit schrumpfbaren Einzeltrack, Terminbox und Card besitzen `min-w-0`/volle begrenzte Breite, und Badge/Metadaten bleiben im normalen Flow. Der Header verwendet die bestehende validierte Social-Datenquelle für optionale mobile Instagram-/Facebook-Icons und ergänzt einen permanenten Anfahrt-Pin; das Termine-Serviceicon bleibt Desktop vorbehalten. Der reale Smartphone-Review ist bestanden.
- Browser-Nacharbeit 20 normalisiert die mobile Headergruppe auf identische 40-Pixel-Touchflächen, einen zentralen gleichmäßigen Abstand und optisch ausgeglichene Instagram-, Facebook- und Standortglyphen. Der Hamburger bleibt als separates Bedienelement unverändert. Der abschließende reale Smartphone- und Responsive-Review ist bestanden.
- Keine Datenbank-, SQL-, Rollen-, Dashboard- oder Inhaltsmigration wurde eingeführt.

## Nächster Schritt

B15.24G ist **COMPLETE – CURRENT FOOTBALL AREA ACCEPTED AS DELIVERED**. Jetzt bearbeitbar ist B15.24H für die fachliche und technische Tischtennisplanung/-integration. Separat zurückgestellt bleiben FUSSBALL.DE-Widgets bis zur finalen registrierten Produktivdomain (**BLOCKED UNTIL FINAL LIVE DOMAIN**) und der vollständige Ausbau von Turniere & Events (**FUTURE / POST-GO-LIVE**). Maps/Consent, weitere Abteilungsintegration sowie Accessibility, Performance und SEO bleiben eigenständig offen.
