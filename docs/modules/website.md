...
Der Footer gliedert sich responsiv in Vereinspräsentation, Verein, Sportarten, weitere interne Links und Kontakt. Vereins-/Kontaktdaten und Social Links stammen aus `club_settings`; ein gemeinsamer Resolver validiert die vorhandenen Keys Facebook, Instagram, YouTube, TikTok, LinkedIn und X für Header und Footer. Rechtlinks für Impressum und Datenschutz erscheinen nur bei publiziertem Inhalt. „Cookie-Einstellungen“ öffnet den zentralen, manuell bestätigten B15.24M-Consent-Dialog; `/cookie-einstellungen` bietet denselben funktionalen Einstieg. `/anfahrt` bezieht Adresse und Google-Maps-Ziel zentral aus `club_settings`; ein Embed bleibt bis zur Einwilligung in externe Inhalte blockiert. Vollständiges Impressum und vollständige Datenschutzerklärung bleiben verbindliche Pre-Go-live-Inhalte. Offen bleiben außerdem verifizierte externe Verbands-/Ortslinks, Vereinsregisterdaten und AGB. NewsCards bleiben unverändert.

Nach manuellem Designreview verwenden die Social Glyphs keine originalfarbigen Markenflächen, sondern dieselben transparenten roten Icon-Tokens wie Standort und Kalender. Eine zukünftige interaktive Karte verwendet die offizielle Google Maps Embed API und die zentrale Vereinsadresse. Der serverseitige Konfigurationsname ist `GOOGLE_MAPS_EMBED_API_KEY`; es wird kein Wert im Repository hinterlegt. Der Key muss in Google Cloud auf die benötigte API und zulässige Website-Referrer beschränkt werden. Ohne gültige Konfiguration wird kein iframe erzeugt. Mit Konfiguration bleibt das iframe bis zur Einwilligung in B15.24M „Externe Inhalte“ gesperrt und verlangt anschließend weiterhin einen bewussten Klick. Google-Maps-Konfiguration und Go-live-Test bleiben separate Aufgaben.
