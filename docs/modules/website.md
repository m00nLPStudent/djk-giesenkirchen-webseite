...
Der Footer gliedert sich responsiv in Vereinspräsentation, Verein, Sportarten, weitere interne Links und Kontakt. Vereins-/Kontaktdaten und Social Links stammen aus `club_settings`; ein gemeinsamer Resolver validiert die vorhandenen Keys Facebook, Instagram, YouTube, TikTok, LinkedIn und X für Header und Footer. Publizierte CMS-Seiten für Impressum, Datenschutz und AGB sind integriert. „Cookie-Einstellungen“ öffnet den zentralen, manuell bestätigten B15.24M-Consent-Dialog; `/cookie-einstellungen` bietet denselben funktionalen Einstieg. `/anfahrt` bezieht Adresse und Google-Maps-Ziel zentral aus `club_settings`; ein Embed bleibt bis zur Einwilligung in externe Inhalte blockiert. Offen bleiben die finale juristische/betriebliche Prüfung der Rechtsinhalte und Anbieter sowie gegebenenfalls verifizierte externe Verbands-/Ortslinks. NewsCards bleiben unverändert.
