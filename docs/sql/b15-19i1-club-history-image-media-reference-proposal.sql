-- B15.19I1 proposal only. Do not execute automatically. No backfill or storage mutation.
BEGIN;

ALTER TABLE public.club_history_images ADD COLUMN IF NOT EXISTS media_asset_id uuid NULL;
ALTER TABLE public.club_history_images ALTER COLUMN image_url DROP NOT NULL;

DO $fk$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.club_history_images'::regclass AND conname='club_history_images_media_asset_id_fkey') THEN
    ALTER TABLE public.club_history_images ADD CONSTRAINT club_history_images_media_asset_id_fkey FOREIGN KEY(media_asset_id) REFERENCES public.media_assets(id) ON DELETE SET NULL;
  END IF;
END;
$fk$;

CREATE INDEX IF NOT EXISTS club_history_images_media_asset_idx ON public.club_history_images(media_asset_id) WHERE media_asset_id IS NOT NULL;
...
BEGIN
  ...
  ELSIF p_entity_type='sponsor' AND p_field_name='image' THEN UPDATE public.sponsors SET image_media_asset_id=p_media_asset_id WHERE id=p_entity_id;
  ELSIF p_entity_type='club_history' AND p_field_name='image' THEN UPDATE public.club_history_images SET media_asset_id=p_media_asset_id WHERE id=p_entity_id;
  END IF;
...
CREATE OR REPLACE FUNCTION public.cleanup_club_history_image_media_usage() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $cleanup$
BEGIN DELETE FROM public.media_asset_usages WHERE entity_type='club_history' AND entity_id=OLD.id AND field_name='image'; RETURN OLD; END;
$cleanup$;
REVOKE ALL ON FUNCTION public.cleanup_club_history_image_media_usage() FROM PUBLIC,anon,authenticated;
DROP TRIGGER IF EXISTS club_history_image_cleanup_media_usage ON public.club_history_images;
CREATE TRIGGER club_history_image_cleanup_media_usage AFTER DELETE ON public.club_history_images FOR EACH ROW EXECUTE FUNCTION public.cleanup_club_history_image_media_usage();
