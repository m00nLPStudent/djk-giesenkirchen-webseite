-- B15.19E3 rollback only. Restores the exact B15.19E2 assignment scope.
BEGIN;
DELETE FROM public.media_asset_usages WHERE entity_type IN ('team','team_season') AND field_name='contact_image';
DROP INDEX IF EXISTS public.teams_contact_image_media_asset_idx;
DROP INDEX IF EXISTS public.team_seasons_contact_image_media_asset_idx;
ALTER TABLE public.teams DROP COLUMN IF EXISTS contact_image_media_asset_id;
ALTER TABLE public.team_seasons DROP COLUMN IF EXISTS contact_image_media_asset_id;
...
BEGIN
  IF p_entity_type NOT IN ('coach','player','board_member','club_contact','team','team_season') OR p_field_name <> 'image' THEN RAISE EXCEPTION 'Unsupported media assignment target'; END IF;
  IF p_media_asset_id IS NOT NULL THEN
    SELECT * INTO v_asset FROM public.media_assets WHERE id=p_media_asset_id;
    IF NOT FOUND OR v_asset.is_archived OR v_asset.media_kind <> 'image' THEN RAISE EXCEPTION 'Media asset is not assignable to %',p_entity_type; END IF;
  END IF;
  ...
  ELSIF p_entity_type='team' THEN UPDATE public.teams SET team_image_media_asset_id=p_media_asset_id WHERE id=p_entity_id;
  ELSE UPDATE public.team_seasons SET team_image_media_asset_id=p_media_asset_id WHERE id=p_entity_id;
  END IF;
