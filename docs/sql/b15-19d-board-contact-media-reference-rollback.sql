-- B15.19D rollback proposal only. Do not execute automatically.
-- Restores the synchronize_media_assignment RPC to the exact B15.19C scope.
-- Does not delete media assets, usage tables, usage rows or storage files.
BEGIN;

DROP INDEX IF EXISTS public.board_members_image_media_asset_idx;
DROP INDEX IF EXISTS public.club_contacts_image_media_asset_idx;
...
BEGIN
  IF p_entity_type NOT IN ('coach', 'player') OR p_field_name <> 'image' THEN
    RAISE EXCEPTION 'Unsupported media assignment target';
  END IF;

  v_expected_purpose := p_entity_type;
  IF p_media_asset_id IS NOT NULL THEN
    SELECT * INTO v_asset FROM public.media_assets WHERE id = p_media_asset_id;
    IF NOT FOUND OR v_asset.is_archived OR v_asset.media_kind <> 'image' OR v_asset.purpose <> v_expected_purpose THEN
