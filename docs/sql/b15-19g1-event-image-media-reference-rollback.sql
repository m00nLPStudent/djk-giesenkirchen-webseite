-- B15.19G1 rollback only. Restores the B15.19F2 assignment scope. No storage deletion.
BEGIN;
DROP TRIGGER IF EXISTS event_cleanup_media_usage ON public.events;
DROP FUNCTION IF EXISTS public.cleanup_event_media_usage();
DELETE FROM public.media_asset_usages WHERE entity_type='event' AND field_name='image';
DROP INDEX IF EXISTS public.events_image_media_asset_idx;
ALTER TABLE public.events DROP COLUMN IF EXISTS image_media_asset_id;
