-- B15.19I1 guarded rollback. Never removes assets or storage objects.
BEGIN;
DO $guard$
BEGIN
  IF EXISTS(SELECT 1 FROM public.club_history_images WHERE media_asset_id IS NOT NULL) THEN RAISE EXCEPTION 'B15.19I1 rollback refused: central club-history references still exist'; END IF;
  IF EXISTS(SELECT 1 FROM public.club_history_images WHERE image_url IS NULL) THEN RAISE EXCEPTION 'B15.19I1 rollback refused: legacy image_url values are incomplete'; END IF;
  IF EXISTS(SELECT 1 FROM public.media_asset_usages WHERE entity_type='club_history') THEN RAISE EXCEPTION 'B15.19I1 rollback refused: club-history usages still exist'; END IF;
END;
$guard$;
DROP TRIGGER IF EXISTS club_history_image_cleanup_media_usage ON public.club_history_images;
DROP FUNCTION IF EXISTS public.cleanup_club_history_image_media_usage();
DROP INDEX IF EXISTS public.club_history_images_media_asset_idx;
ALTER TABLE public.club_history_images DROP COLUMN IF EXISTS media_asset_id;
ALTER TABLE public.club_history_images ALTER COLUMN image_url SET NOT NULL;
...
BEGIN
  IF NOT ((p_field_name='image' AND p_entity_type IN('coach','player','board_member','club_contact','team','team_season','news','event','sponsor')) OR (p_field_name='contact_image' AND p_entity_type IN('team','team_season')) OR (p_field_name='file' AND p_entity_type IN('news_document','event_document'))) THEN RAISE EXCEPTION 'Unsupported media assignment target'; END IF;
