-- B15.19E3 proposal only. Do not execute automatically. No backfill or legacy mutation.
BEGIN;

ALTER TABLE public.teams ADD COLUMN IF NOT EXISTS contact_image_media_asset_id uuid NULL;
ALTER TABLE public.team_seasons ADD COLUMN IF NOT EXISTS contact_image_media_asset_id uuid NULL;

DO $e3_fk$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.teams'::regclass AND conname='teams_contact_image_media_asset_id_fkey') THEN
    ALTER TABLE public.teams ADD CONSTRAINT teams_contact_image_media_asset_id_fkey FOREIGN KEY (contact_image_media_asset_id) REFERENCES public.media_assets(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.team_seasons'::regclass AND conname='team_seasons_contact_image_media_asset_id_fkey') THEN
    ALTER TABLE public.team_seasons ADD CONSTRAINT team_seasons_contact_image_media_asset_id_fkey FOREIGN KEY (contact_image_media_asset_id) REFERENCES public.media_assets(id) ON DELETE SET NULL;
  END IF;
END;
$e3_fk$;

CREATE INDEX IF NOT EXISTS teams_contact_image_media_asset_idx ON public.teams(contact_image_media_asset_id) WHERE contact_image_media_asset_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS team_seasons_contact_image_media_asset_idx ON public.team_seasons(contact_image_media_asset_id) WHERE contact_image_media_asset_id IS NOT NULL;
...
BEGIN
  IF NOT (
    (p_field_name = 'image' AND p_entity_type IN ('coach','player','board_member','club_contact','team','team_season')) OR
    (p_field_name = 'contact_image' AND p_entity_type IN ('team','team_season'))
  ...
  ELSE UPDATE public.team_seasons SET contact_image_media_asset_id=p_media_asset_id WHERE id=p_entity_id;
  END IF;
