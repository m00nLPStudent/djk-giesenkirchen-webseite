-- B15.19H1 proposal only. Do not execute automatically. No backfill or storage mutation.
BEGIN;

ALTER TABLE public.sponsors ADD COLUMN IF NOT EXISTS image_media_asset_id uuid NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid='public.sponsors'::regclass
      AND conname='sponsors_image_media_asset_id_fkey'
  ) THEN
    ALTER TABLE public.sponsors
      ADD CONSTRAINT sponsors_image_media_asset_id_fkey
      FOREIGN KEY(image_media_asset_id) REFERENCES public.media_assets(id) ON DELETE SET NULL;
  END IF;
END;
$fk$;

CREATE INDEX IF NOT EXISTS sponsors_image_media_asset_idx
  ON public.sponsors(image_media_asset_id) WHERE image_media_asset_id IS NOT NULL;
...
BEGIN
  IF NOT (
    (p_field_name='image' AND p_entity_type IN('coach','player','board_member','club_contact','team','team_season','news','event','sponsor')) OR
...
CREATE OR REPLACE FUNCTION public.cleanup_sponsor_media_usage() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $cleanup$
BEGIN DELETE FROM public.media_asset_usages WHERE entity_type='sponsor' AND entity_id=OLD.id AND field_name='image'; RETURN OLD; END;
$cleanup$;
REVOKE ALL ON FUNCTION public.cleanup_sponsor_media_usage() FROM PUBLIC,anon,authenticated;
DROP TRIGGER IF EXISTS sponsor_cleanup_media_usage ON public.sponsors;
CREATE TRIGGER sponsor_cleanup_media_usage AFTER DELETE ON public.sponsors FOR EACH ROW EXECUTE FUNCTION public.cleanup_sponsor_media_usage();
