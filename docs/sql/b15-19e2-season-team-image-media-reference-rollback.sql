-- B15.19E2 rollback proposal only. Restores the E1 assignment scope.
BEGIN;

DELETE FROM public.media_asset_usages
WHERE entity_type = 'team_season' AND field_name = 'image';

DROP INDEX IF EXISTS public.team_seasons_team_image_media_asset_idx;
ALTER TABLE public.team_seasons DROP COLUMN IF EXISTS team_image_media_asset_id;
...
BEGIN
  IF p_entity_type NOT IN ('coach', 'player', 'board_member', 'club_contact', 'team')
     OR p_field_name <> 'image' THEN RAISE EXCEPTION 'Unsupported media assignment target'; END IF;
  ...
  ELSE UPDATE public.teams SET team_image_media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  END IF;
