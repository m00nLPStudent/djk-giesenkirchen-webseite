-- B15.19F2 proposal only. Do not execute automatically. No backfill.
BEGIN;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'public.news_documents'::regclass
      AND conname = 'news_documents_media_asset_id_fkey'
  ) THEN
    ALTER TABLE public.news_documents
      ADD CONSTRAINT news_documents_media_asset_id_fkey
...
CREATE INDEX IF NOT EXISTS news_documents_media_asset_idx
  ON public.news_documents(media_asset_id)
...
BEGIN
  ...
  IF v_constraint_name IS NOT NULL THEN
    EXECUTE format(
      'ALTER TABLE public.media_asset_usages DROP CONSTRAINT %I',
      v_constraint_name
    );
  END IF;
  ...
  ALTER TABLE public.media_asset_usages
    ...
    CHECK (entity_type IN (
      ...
      'team_season', 'news', 'news_document', 'page', 'club_history',
...
BEGIN
  IF NOT (
    (p_field_name = 'image' AND p_entity_type IN (
      'coach', 'player', 'board_member', 'club_contact',
      'team', 'team_season', 'news'
    ))
    OR (p_field_name = 'contact_image' AND p_entity_type IN (
      'team', 'team_season'
    ))
    OR (p_field_name = 'file' AND p_entity_type = 'news_document')
  ) THEN
    RAISE EXCEPTION 'Unsupported media assignment target';
  END IF;

  v_expected_media_kind := CASE
    WHEN p_field_name = 'file' THEN 'document'
    ELSE 'image'
  END;
  ...
  IF p_media_asset_id IS NOT NULL THEN
    ...
    IF v_asset.is_archived THEN
      RAISE EXCEPTION 'Archived media asset is not assignable';
    END IF;
    IF v_asset.media_kind <> v_expected_media_kind THEN
      RAISE EXCEPTION 'Media asset is not assignable to %/%',
        p_entity_type, p_field_name;
    END IF;
  END IF;
  ...
  ELSIF p_entity_type = 'team' AND p_field_name = 'image' THEN
    UPDATE public.teams SET team_image_media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  ELSIF p_entity_type = 'team_season' AND p_field_name = 'image' THEN
    UPDATE public.team_seasons SET team_image_media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  ELSIF p_entity_type = 'team' AND p_field_name = 'contact_image' THEN
    UPDATE public.teams SET contact_image_media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  ELSIF p_entity_type = 'team_season' AND p_field_name = 'contact_image' THEN
    UPDATE public.team_seasons SET contact_image_media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  ELSIF p_entity_type = 'news' AND p_field_name = 'image' THEN
    UPDATE public.news SET image_media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  ELSIF p_entity_type = 'news_document' AND p_field_name = 'file' THEN
    UPDATE public.news_documents SET media_asset_id = p_media_asset_id WHERE id = p_entity_id;
  END IF;
  ...
  IF p_media_asset_id IS NOT NULL THEN
    INSERT INTO public.media_asset_usages(
      media_asset_id, entity_type, entity_id, field_name
    ) VALUES (
      p_media_asset_id, p_entity_type, p_entity_id, p_field_name
    );
  END IF;
...
CREATE OR REPLACE FUNCTION public.cleanup_news_document_media_usage()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  DELETE FROM public.media_asset_usages
  WHERE entity_type = 'news_document'
...
REVOKE ALL ON FUNCTION public.cleanup_news_document_media_usage()
  FROM PUBLIC, anon, authenticated;
DROP TRIGGER IF EXISTS news_document_cleanup_media_usage ON public.news_documents;
CREATE TRIGGER news_document_cleanup_media_usage
  AFTER DELETE ON public.news_documents
  FOR EACH ROW
  EXECUTE FUNCTION public.cleanup_news_document_media_usage();
