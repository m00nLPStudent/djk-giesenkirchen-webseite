-- B15.19G1 proposal only. Do not execute automatically. No backfill or legacy mutation.
BEGIN;

ALTER TABLE public.events ADD COLUMN IF NOT EXISTS image_media_asset_id uuid NULL;

DO $fk$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.events'::regclass AND conname = 'events_image_media_asset_id_fkey') THEN
    ALTER TABLE public.events ADD CONSTRAINT events_image_media_asset_id_fkey
      FOREIGN KEY (image_media_asset_id) REFERENCES public.media_assets(id) ON DELETE SET NULL;
  END IF;
END;
$fk$;

CREATE INDEX IF NOT EXISTS events_image_media_asset_idx ON public.events(image_media_asset_id) WHERE image_media_asset_id IS NOT NULL;
...
BEGIN
  IF NOT (
    ...
    (p_field_name='file' AND p_entity_type='news_document')
...
CREATE OR REPLACE FUNCTION public.cleanup_event_media_usage() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $cleanup$
BEGIN
  DELETE FROM public.media_asset_usages WHERE entity_type='event' AND entity_id=OLD.id AND field_name='image';
  RETURN OLD;
END;
$cleanup$;
REVOKE ALL ON FUNCTION public.cleanup_event_media_usage() FROM PUBLIC,anon,authenticated;
DROP TRIGGER IF EXISTS event_cleanup_media_usage ON public.events;
CREATE TRIGGER event_cleanup_media_usage AFTER DELETE ON public.events FOR EACH ROW EXECUTE FUNCTION public.cleanup_event_media_usage();
