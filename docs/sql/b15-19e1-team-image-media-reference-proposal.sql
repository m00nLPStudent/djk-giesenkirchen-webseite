-- B15.19E1 proposal only. Do not execute automatically. No data migration.
BEGIN;
...
CREATE INDEX IF NOT EXISTS teams_team_image_media_asset_idx
  ON public.teams(team_image_media_asset_id);
...
BEGIN
  IF p_entity_type NOT IN ('coach', 'player', 'board_member', 'club_contact', 'team') OR p_field_name <> 'image' THEN
