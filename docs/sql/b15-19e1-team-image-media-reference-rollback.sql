-- B15.19E1 rollback proposal only. Restores the B15.19D2/D3 RPC scope.
-- Does not delete media assets, storage files, season fields or contact fields.
BEGIN;

DELETE FROM public.media_asset_usages
WHERE entity_type = 'team' AND field_name = 'image';

DROP INDEX IF EXISTS public.teams_team_image_media_asset_idx;
ALTER TABLE public.teams DROP COLUMN IF EXISTS team_image_media_asset_id;
