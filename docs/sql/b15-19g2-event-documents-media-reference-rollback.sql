-- B15.19G2 guarded rollback. Restores G1/F2 contracts and never deletes central references.
BEGIN;
DO $guard$
BEGIN
  IF EXISTS(SELECT 1 FROM public.event_documents WHERE media_asset_id IS NOT NULL) THEN
    RAISE EXCEPTION 'B15.19G2 rollback refused: central event document references still exist';
  END IF;
  IF EXISTS(SELECT 1 FROM public.event_documents WHERE file_path IS NULL OR file_url IS NULL) THEN
    RAISE EXCEPTION 'B15.19G2 rollback refused: legacy NOT NULL fields are incomplete';
  END IF;
  IF EXISTS(SELECT 1 FROM public.media_asset_usages WHERE entity_type='event_document') THEN
    RAISE EXCEPTION 'B15.19G2 rollback refused: event_document usages still exist';
  END IF;
END;
$guard$;
DROP TRIGGER IF EXISTS event_document_cleanup_media_usage ON public.event_documents;
DROP FUNCTION IF EXISTS public.cleanup_event_document_media_usage();
DROP INDEX IF EXISTS public.event_documents_media_asset_idx;
ALTER TABLE public.event_documents DROP COLUMN IF EXISTS media_asset_id;
ALTER TABLE public.event_documents ALTER COLUMN file_path SET NOT NULL;
ALTER TABLE public.event_documents ALTER COLUMN file_url SET NOT NULL;
ALTER TABLE public.media_asset_usages DROP CONSTRAINT IF EXISTS media_asset_usages_entity_type_check;
ALTER TABLE public.media_asset_usages ADD CONSTRAINT media_asset_usages_entity_type_check CHECK(entity_type IN('player','coach','board_member','club_contact','team','team_season','news','news_document','event','page','club_history','sponsor','document','download','system'));
...
BEGIN
  IF NOT((p_field_name='image' AND p_entity_type IN('coach','player','board_member','club_contact','team','team_season','news','event')) OR (p_field_name='contact_image' AND p_entity_type IN('team','team_season')) OR (p_field_name='file' AND p_entity_type='news_document')) THEN RAISE EXCEPTION 'Unsupported media assignment target'; END IF;
