-- B15.19F1 proposal only. Do not execute automatically. No backfill or legacy mutation.
BEGIN;

ALTER TABLE public.news ADD COLUMN IF NOT EXISTS image_media_asset_id uuid NULL;

DO $f1_fk$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.news'::regclass AND conname='news_image_media_asset_id_fkey') THEN
    ALTER TABLE public.news ADD CONSTRAINT news_image_media_asset_id_fkey FOREIGN KEY (image_media_asset_id) REFERENCES public.media_assets(id) ON DELETE SET NULL;
  END IF;
END;
$f1_fk$;

CREATE INDEX IF NOT EXISTS news_image_media_asset_idx ON public.news(image_media_asset_id) WHERE image_media_asset_id IS NOT NULL;
...
BEGIN
  IF NOT (
    (p_field_name='image' AND p_entity_type IN ('coach','player','board_member','club_contact','team','team_season','news')) OR
...
CREATE TRIGGER news_cleanup_media_usage AFTER DELETE ON public.news FOR EACH ROW EXECUTE FUNCTION public.cleanup_news_media_usage();
