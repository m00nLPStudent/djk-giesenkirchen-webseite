-- B15.19C proposal only. Do not execute automatically. No existing-data migration.
BEGIN;

ALTER TABLE public.players
  ADD COLUMN image_media_asset_id uuid NULL REFERENCES public.media_assets(id) ON DELETE SET NULL;

CREATE INDEX players_image_media_asset_idx
  ON public.players (image_media_asset_id)
