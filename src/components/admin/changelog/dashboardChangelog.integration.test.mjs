...
test("version and visible copy have a single central source", () => {
  assert.match(config, /version: "1\.0\.6"/);
  assert.match(config, /releaseDate: "2026-09-24"/);
  assert.match(config, /Vorstandsaufgaben & optimierte Lizenzanzeige/);
  assert.match(config, /Aufgaben und Zuständigkeiten/);
  assert.match(config, /Keine Lizenz/);
  assert.doesNotMatch(gate, /version:\s*"1\.0\.6"/);
  assert.doesNotMatch(dialog, /version:\s*"1\.0\.6"/);
});
