...
export async function listMediaAssets(db, { search = "", kind = "all", visibility = "all", purpose = "all", mimeType = "all", storageBucket = "all", archived = "active", sort = "newest", page = 1, pageSize = 100 } = {}) {
  ...
  let query = db.from("media_assets").select(MEDIA_ASSET_SELECT, { count: "exact" }).range(from, from + safePageSize - 1);
...
export function insertMediaAsset(db, payload) { return db.from("media_assets").insert(payload).select(MEDIA_ASSET_SELECT).single(); }
...
export function loadMediaAsset(db, id) { return db.from("media_assets").select(MEDIA_ASSET_SELECT).eq("id", id).maybeSingle(); }
export function loadMediaAssets(db, ids) { return db.from("media_assets").select(MEDIA_ASSET_SELECT).in("id", ids); }
