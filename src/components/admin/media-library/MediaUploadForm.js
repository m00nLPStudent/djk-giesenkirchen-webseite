...
export default function MediaUploadForm() {
  ...
  return <form action={action} className="grid gap-4 rounded-3xl border border-white/10 bg-white/5 p-5 md:grid-cols-2">
    ...
    <label className="space-y-2 text-sm font-bold text-white">Verwendung<select name="purpose" defaultValue="document" className="block w-full rounded-2xl border border-white/10 bg-neutral-900 p-3">{["player","coach","board","team","news","cms","club_history","sponsor","event","document","download","system"].map((value)=><option key={value}>{value}</option>)}</select></label>
