...
export default function AdminMediaPicker({ value, legacyUrl, placeholderUrl, placeholder, onChange, loadAction, uploadAction, usageContext = "coach", defaultPurpose = getDefaultPurposeForUsageContext(usageContext), entityLabel = "Trainerbild" }) {
  ...
  return <div className="grid gap-5 md:grid-cols-[180px_1fr] md:items-center">
    <div className="flex h-40 w-40 items-center justify-center overflow-hidden rounded-3xl bg-black/20 ring-1 ring-white/10">{previewUrl ? <img src={previewUrl} alt={value?.displayName || entityLabel} className="h-full w-full object-cover"/> : placeholder || <span className="text-sm text-white/40">Kein Bild</span>}</div>
    ...
    <AdminMediaPickerDialog open={open} onClose={()=>setOpen(false)} onSelect={onChange} loadAction={loadAction} uploadAction={uploadAction} defaultPurpose={defaultPurpose} fixedScope={usageContext === "profile"}/>
  </div>;
}
