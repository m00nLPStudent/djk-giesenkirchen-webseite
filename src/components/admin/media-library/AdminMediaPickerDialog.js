...
export default function AdminMediaPickerDialog({ open, onClose, onSelect, loadAction, uploadAction, mediaKind = "image", allowUpload = true }) {
  ...
  useEffect(() => {
    ...
    if (open && dialog && !dialog.open) { dialog.showModal(); void load(defaults); setTimeout(() => searchRef.current?.focus(), 0); }
