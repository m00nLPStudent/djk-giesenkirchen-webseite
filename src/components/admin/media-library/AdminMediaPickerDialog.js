...
const PAGE_SIZE = 12;
const defaults = { search: "", visibility: "all", purpose: "coach", page: 1, pageSize: PAGE_SIZE };

export default function AdminMediaPickerDialog({ open, onClose, onSelect, loadAction, uploadAction, mediaKind = "image", usageContext = "coach", allowUpload = true }) {
  const dialogRef = useRef(null);
  const searchRef = useRef(null);
  const [filters, setFilters] = useState({ ...defaults, purpose: usageContext });
  ...
  async function load(next = filters) {
    setLoading(true);
    const response = await loadAction({ ...next, kind: mediaKind, purpose: usageContext });
  ...
  useEffect(() => {
    ...
    if (open && dialog && !dialog.open) { const initial = { ...defaults, purpose: usageContext }; setFilters(initial); dialog.showModal(); void load(initial); setTimeout(() => searchRef.current?.focus(), 0); }
    if (!open && dialog?.open) dialog.close();
  // Opening deliberately resets filters; action identities are stable Server Action references.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open]);

  function close() { dialogRef.current?.close(); onClose(); }
  async function submitFilters(event) { event?.preventDefault(); event?.stopPropagation(); const next = { ...filters, page: 1 }; setFilters(next); await load(next); }
  async function go(page) { const next = { ...filters, page }; setFilters(next); await load(next); }
  async function upload(event) {
    const file = event.target.files?.[0]; event.target.value = ""; if (!file) return;
    setUploading(true); const data = new FormData(); data.set("file", file); data.set("displayName", file.name); data.set("altText", "");
  ...
  return <dialog ref={dialogRef} aria-labelledby="media-picker-title" onCancel={(event) => { event.preventDefault(); close(); }} onClose={onClose} className="m-auto max-h-[92vh] w-[min(96vw,72rem)] rounded-3xl border border-white/15 bg-zinc-950 p-0 text-white backdrop:bg-black/80">
    <div className="flex max-h-[92vh] flex-col"><header className="flex items-center justify-between border-b border-white/10 p-5"><div><h2 id="media-picker-title" className="text-xl font-black">Medium auswählen</h2><p className="mt-1 text-sm text-white/50">Ein vorhandenes Bild wählen oder direkt neu hochladen.</p></div><button type="button" onClick={close} className="rounded-full border border-white/15 px-4 py-2 font-bold">Abbrechen</button></header>
      <div className="overflow-y-auto p-5"><div onKeyDown={(event)=>{if(event.key==="Enter"&&event.target===searchRef.current){void submitFilters(event);}}} className="grid gap-3 sm:grid-cols-4"><input ref={searchRef} value={filters.search} onChange={(e)=>setFilters((v)=>({...v,search:e.target.value}))} placeholder="Name, Datei, Alttext, Beschreibung …" className="rounded-2xl border border-white/10 bg-black/20 p-3 sm:col-span-2"/><select value={filters.visibility} onChange={(e)=>setFilters((v)=>({...v,visibility:e.target.value}))} className="rounded-2xl border border-white/10 bg-neutral-900 p-3"><option value="all">Alle Sichtbarkeiten</option><option value="public">Öffentlich</option><option value="admin">Admin</option></select><select value={filters.purpose} disabled className="rounded-2xl border border-white/10 bg-neutral-900 p-3"><option value={usageContext}>{usageContext === "player" ? "Spieler" : "Trainer"}</option></select><button type="button" onClick={submitFilters} className="w-fit rounded-full border border-white/15 px-5 py-2 font-bold">Suchen</button>{allowUpload ? <label className="w-fit cursor-pointer rounded-full bg-red-600 px-5 py-2 font-bold"><input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={uploading} onChange={upload}/>{uploading ? "Upload läuft …" : "Neues Bild hochladen"}</label> : null}</div>
