const TARGETS = new Set(["coach", "player", "board_member", "club_contact", "team", "team_season", "news", "news_document", "event", "event_document", "sponsor", "club_history"]);
...
export function buildMediaAssignmentPayload(entityType, entityId, mediaAssetId, fieldName = "image") {
  const validField = (fieldName === "image" && !["news_document", "event_document"].includes(entityType)) ||
    (fieldName === "contact_image" && CONTACT_IMAGE_TARGETS.has(entityType)) ||
    (fieldName === "file" && ["news_document", "event_document"].includes(entityType));
