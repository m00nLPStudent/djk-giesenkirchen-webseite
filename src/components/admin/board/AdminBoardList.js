...
function ScopedLink({ member, children }) {
  const href = member._canEditInScope === false ? null : `/admin/department/board/edit/${member.id}`;
  return <Can permission="settings.view" uiOnly fallback={children(null)}>{children(href)}</Can>;
}

function BoardRow({ member, href }) {
  const name = getBoardMemberName(member);
  return <AdminListRow href={href} label={href ? `${name} öffnen` : undefined} template={TEMPLATE}><BoardMemberAvatar member={member} /><span className="truncate font-black text-white">{name}</span><span className="truncate text-white/65">{member.role_de || "–"}</span><span className="truncate text-white/55">Fußballabteilung</span><BoardMemberStatus member={member} />{href ? <AdminListChevron label={`${name} öffnen`} /> : <span />}</AdminListRow>;
}

function BoardMobileCard({ member, href }) {
  const name = getBoardMemberName(member);
  return <AdminListMobileCard href={href} label={href ? `${name} öffnen` : undefined}><div className="flex items-start gap-4"><BoardMemberAvatar member={member} sizeClassName="h-16 w-16" /><div className="min-w-0 flex-1"><h2 className="break-words text-lg font-black text-white">{name}</h2><p className="mt-1 truncate text-sm text-white/60">{member.role_de || "Keine Funktion"}</p><p className="mt-1 text-xs text-white/40">Fußballabteilung</p><div className="mt-3"><BoardMemberStatus member={member} /></div></div>{href ? <AdminListChevron label={`${name} öffnen`} /> : null}</div></AdminListMobileCard>;
}

export default function AdminBoardList({ members = [], canCreate = false }) {
