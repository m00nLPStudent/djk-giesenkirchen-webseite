...
export default function PlayerContributionDetailView({ player, canEdit = false, canArchive = false, contributionStatus = null, contributionVisibility = "none", contributionSeasonWarning = "" }) {
  const teamLabels = player.teamNames?.length ? player.teamNames.join(" · ") : "Keine Mannschaft";
  ...
  const headerActions = hasHeaderActions ? (
    <AdminActionBar>
      ...
      {canEdit ? <AdminButton href={`/admin/players/edit/${player.id}`} variant="primary">Bearbeiten</AdminButton> : null}
    </AdminActionBar>
  ) : null;
  ...
  return (
    <AdminDetailLayout
      header={<AdminDetailHeader backHref="/admin/players" backLabel="Zurück zu Spielern" backVariant="pill" title={player.displayName} leading={<PlayerAvatar player={player} sizeClassName="h-16 w-16" />} status={<PlayerStatusBadge active={player.isActive} />} meta={`${teamLabels} · ${seasonLabel}`} actions={headerActions} />}
