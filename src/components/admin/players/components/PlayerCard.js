      ...
      <div className="mt-4 flex flex-wrap items-end gap-4">
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-white/45">
            <span>{player.year_group ? `Jahrgang ${player.year_group}` : "Jahrgang nicht hinterlegt"}</span>
            {player.strong_foot && <span>• {player.strong_foot}</span>}
          </div>
        </div>
      </div>

      {player.description_de && <p className="mt-4 max-w-3xl text-white/60">{player.description_de}</p>}

      <EntityCardActions>
        <EntityActionLink href={`/admin/players/edit/${player.id}`}>Bearbeiten</EntityActionLink>
        <EntityActionLink href={profileUrl} target="_blank" variant="primary">Profil anzeigen</EntityActionLink>
        <AdminRemoveButton
          label="Spieler"
          name={fullName}
          action={() => removePlayerRecord(player)}
          affected={["Spielerprofil", "Kader-Zuordnungen", "Spielerbild, sofern vorhanden"]}
