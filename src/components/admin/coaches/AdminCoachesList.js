...
function departmentLabel(coach) {
  return coach.departmentName || "Nicht zugeordnet";
}

function coachHref(coach, basePath = "/admin/coaches") {
  return coach._canEditInScope === false
    ? `/trainer/${coach.slug}`
    : `${basePath}/edit/${coach.id}`;
}

export default function AdminCoachesList({ coaches = [], basePath = "/admin/coaches" }) {
  if (!coaches.length) return <CoachEmptyState />;
  return (
    <div className="space-y-3">
      {coaches.map((coach) => <CoachCard key={`${coach.id}-mobile`} coach={coach} basePath={basePath} />)}
      <AdminCard className="hidden overflow-hidden lg:block">
        ...
        {coaches.map((coach) => (
          <Link key={coach.id} href={coachHref(coach, basePath)} className="grid grid-cols-[3.5rem_minmax(10rem,1.25fr)_minmax(8rem,0.8fr)_minmax(10rem,1fr)_7rem_4rem] items-center gap-4 border-t border-white/10 px-5 py-3 text-sm transition hover:bg-white/[0.05] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-red-400">
            ...
            <span className="min-w-0"><span className="block truncate text-white/65">{teamLabel(coach)}</span><span className="mt-1 block truncate text-xs text-white/40">{departmentLabel(coach)}</span></span>
