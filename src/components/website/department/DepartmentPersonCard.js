    ...
    <article className="overflow-hidden rounded-3xl border border-white/10 bg-white/5">
      ...
      <div className="p-6">
        ...
        <div className="mt-6 flex gap-3">
          {phoneHref && (
            <a href={phoneHref} aria-label={`${fullName} anrufen`} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/70 transition hover:border-red-500 hover:bg-red-600 hover:text-white">
              <Phone size={18} />
            </a>
          )}
          {person.email && (
            <a href={`mailto:${person.email}`} aria-label={`${fullName} eine E-Mail schreiben`} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/70 transition hover:border-red-500 hover:bg-red-600 hover:text-white">
              <Mail size={18} />
            </a>
          )}
        </div>
