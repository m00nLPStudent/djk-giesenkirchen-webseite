...
export default async function AdminWomenGymnasticsPage() {
  ...
  return <AdminLayout title={config.label} subtitle="Adminbereich" showHeader={false}>
    <AdminModulePage>
      <AdminModuleHeader eyebrow="Abteilungsseite" title={config.label} description="Inhalte, Ansprechpartner, Gruppenbild und teamunabhängige Trainingszeiten pflegen."/>
      {loaded.error ? <div role="alert" className="rounded-3xl border border-red-500/30 bg-red-500/10 p-6 text-red-100">Der Gymnastikdamen-Bereich konnte nicht geladen werden. Bitte die Seite neu laden.</div> : <DepartmentSectionEditor config={config} initialSection={loaded.data.section} initialTrainingTimes={loaded.data.trainingTimes} actions={actions}/>}
    </AdminModulePage>
  </AdminLayout>;
}
